/**
 * @file logger.js
 * A lightweight logging utility for CLI output using ANSI escape codes.
 *
 * Features:
...
/**
 * Intensity levels for colors.
 * @typedef {"dim" | "normal" | "intense"} Intensity
 */

/**
 * ANSI escape codes grouped by color and intensity.
 *
 * @type {Object<string, Record<Intensity, string>>}
 */
const colorPalette = {
    black: {
...
/**
 * Predefined semantic log styles.
 *
...
/**
 * Resolve a custom color style string like `"red.intense"`.
 *
 * @param {string} logColorStyle - Style string in format `"color.intensity"`.
 * @returns {string} ANSI escape code for the matching color/intensity.
 *
 * @example
 ...
 *   - A predefined log type (`info`, `success`, `error`, `warn`, `response`, `debug`), or
 *   - A custom style in `"color.intensity"` format (e.g., `"blue.dim"`).
 * @param {string} logText - The message to display.
 * @returns {void}
 *
 * @example
